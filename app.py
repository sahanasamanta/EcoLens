...
st.set_page_config(layout="wide", page_title="Plastic Waste — Citizen Reports & NGO Dashboard")
...
if mode == "Report (Citizen)":
    ...
    if st.button("Submit report"):
        if uploaded is None:
            ...
            if vis_type == "Heatmap Overlay":
                st.image(pil_from_cv2(overlay), caption="Heatmap overlay", use_container_width=True)
            elif vis_type == "Predicted Bounding Boxes":
                st.image(pil_from_cv2(ann), caption="Predicted Bounding Boxes", use_container_width=True)
    ...
    if not st.session_state["ngo_authenticated"]:
        ...
        if st.button("Submit"):
            if password_input == "ngo2025":  # replace with your password
